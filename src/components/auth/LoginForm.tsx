...
export function LoginForm({ onSwitchToRegister }: { onSwitchToRegister: () => void }) {
  ...
  return (
    <motion.form key="login" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button variant="secondary" onClick={() => {}} className="w-full h-12">使用 Steam 登录</Button>
    </motion.form>
  )
}
